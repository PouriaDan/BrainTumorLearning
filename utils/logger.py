...
class Logger:
    def __init__(self, name, log_file, console_output=True, 
    ...
    def log_training(self, epoch, loss, metrics_dict, lr):
        metrics_str = ", ".join([f"{name.capitalize()}: {value:.4f}" for name, value in metrics_dict.items()])
        self.logger.info(f"[Epoch: {epoch:03d}]      Train info: Loss: {loss:.4f}, {metrics_str}, LR: {lr:.6f}")

    def log_validation(self, epoch, loss, metrics_dict):
        metrics_str = ", ".join([f"{name.capitalize()}: {value:.4f}" for name, value in metrics_dict.items()])
        self.logger.info(f"[Epoch: {epoch:03d}] Validation info: Loss: {loss:.4f}, {metrics_str}")

    def log_test(self, epoch, loss, metrics_dict):
        metrics_str = ", ".join([f"{name.capitalize()}: {value:.4f}" for name, value in metrics_dict.items()])
        self.logger.info(f"[Epoch: {epoch:03d}]       Test info: Loss: {loss:.4f}, {metrics_str}")
